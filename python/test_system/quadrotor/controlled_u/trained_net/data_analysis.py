...
flight_data = np.load('./python/test_system/quadrotor/controlled_u/trained_net/flight_data.npy', allow_pickle=True).item()
...
def update_lines(num, dataLines, lines):
        for line, data, index in zip(lines, dataLines, range(0,data_plots[0][0].size)):
                # NOTE: there is no .set_data() for 3 dim data...
                line.set_data(data[0:2, :num])
                line.set_3d_properties(data[2, :num])
                ax.set_title(u'Flight at t = {}'.format(index*par.DELTA_T))
        return line
...
fig2.savefig('./python/test_system/quadrotor/controlled_u/trained_net/flight_data.png')
...
fig3.savefig('./python/test_system/quadrotor/controlled_u/trained_net/reward_history.png')
...
refx = ref_trajectory[:index_step,0]
refy = ref_trajectory[:index_step,1]
refz = ref_trajectory[:index_step,2]

data_plots = [np.array([x,y,z]), ref_trajectory[:index_step,:].T]
lines = [ax.plot(x, y, z, label='Simulation')[0], ax.plot(refx, refy, refz, 'r--', label='Reference')[0]]
...
line_ani = animation.FuncAnimation(fig1, update_lines, x.size, fargs=(data_plots, lines),
