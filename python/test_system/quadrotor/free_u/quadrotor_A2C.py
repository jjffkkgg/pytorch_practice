...
class Environment:
    ...
    def run(self, hover_time: int) -> None:
        ...
        envs = [QuadRotorEnv(par.space_lim) for i in range(NUM_PROCESSES)]
                    ...
                    if not rec_stop[i]:
                        ...
                        if done_np[i]:          # success or fail
                            ...
                            if done_info_np[i,0]:                               # done with hovering
                                ...
                                reward_np[i] = np.clip(1/distance_step[i],0,50)*50
                            elif done_info_np[i,1]:
                                reward_np[i] = 10-vel_n_step[i]
                            else:
                                reward_replay_buffer[i, each_step[i]] = -10
                                reward_np[i] = -hover_time + each_step[i]*DELTA_T
