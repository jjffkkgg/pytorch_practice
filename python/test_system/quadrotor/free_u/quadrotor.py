...
class QuadRotorEnv:
    ...
    def __init__(self, lim: list) -> None:
        ...
        self.done_threshold = np.array([
            4*ca.pi, 4*ca.pi, 4*ca.pi,        # [rad/s]
            5,5,5,                             # [m/s]
            4*ca.pi/18, 4*ca.pi/18, 4*ca.pi/18,      # [rad]
            lim[0],lim[1],lim[2]                    # [m]
        ])

        self.ub_sys = self.done_threshold + par.ref_state
        self.lb_sys = -self.done_threshold + par.ref_state

        # Random noise/force/initial state
        start_angle_deg = par.ref_state[6:9] + np.random.uniform(-1,1,3) * par.max_start_angle_deg
        self.start_angle_rad = start_angle_deg*(np.pi/180)
        rand_force = np.random.uniform(-1,1,3) * par.max_rand_force
        rand_moment = np.random.uniform(-1,1,3) * par.max_rand_moment
        ...
        # state (x)
        self.state_comment = {
            0:'Body roll angular velocity',
            1:'Body pitch angular velocity',
            2:'Body yaw angular velocity',
            3:'Body x velocity',
            4:'Body y velocity',
            5:'Body z velocity',
            6:'Roll angle',
        ...
        # apply random force-moment (ex) turbulent flow
        F_b += rand_force
        M_b += rand_moment
    ...
    def step(self, action: int, step, dt=DELTA_T):
        ...
        distance_vect = par.ref_state[9:12] - self.xi[9:12]
        ...
        for i in range(len(self.xi)):
            if (self.xi[i] >= self.ub_sys[i]) or (self.xi[i] <= self.lb_sys[i]):
                done = True
                print('------over the limit in %s------' % self.state_comment[i]) 
        ...
        return self.xi, self.u, reward, done, np.array([hover, stay]), distance_vect, vel_n.T

    def reset(self, p, hover_time: int):
        '''reset the environment. (init state)'''
        self.xi = [0] * 12
        self.xi[6:9] = self.start_angle_rad
        self.xi[9:12] = par.ref_state[9:12]
        self.steps_beyond_done = None
        self.p = p
